...
GUILDA_ID = "50001"
SERVER_NAME = "GUEL_FF20_SERVERV16"
...
class FreeFireV16Handler(http.server.BaseHTTPRequestHandler):
    def get_db(self):
        default_db = {
    ...
    def do_POST(self):
        ...
        if "login" in self.path or "guest_login" in self.path:
            ...
            response = {
                ...
                "session_key": "guelff_v16_render_" + hashlib.md5(player_id.encode()).hexdigest()[:8]
            }
    ...
    def do_GET(self):
        if any(x in self.path for x in ["ver.php", "version", "free-fire-q96x"]):
            self.send_json_response(200, {"status": 0, "version": "1.15.0"})
            return
        self.send_json_response(404, {"status": 1, "message": "Not Found"})
...
if __name__ == "__main__":
    ...
    print(f"====================================================")
    with ReusableTCPServer(("", PORT), FreeFireV16Handler) as httpd:
        httpd.serve_forever()
