...
# =========================================================================
# !!! TRAVA DE IP DESATIVADA PARA FACILITAR O ACESSO !!!
MEU_IP_ATUAL = None # TRAVA REMOVIDA
# =========================================================================
...
class FreeFireMockHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # LOGIN E REGISTRO SEM TRAVA DE IP
        if "guest_login" in self.path or "login" in self.path:
            ...
            player_id = None
            # Tenta encontrar conta pelo device_id
            ...
            if not player_id:
                ...
                print(f">>> [LOGIN] Jogador {db['players'][player_id]['nickname']} ({player_id}) logou. <<<")

            # Adiciona à guilda e faz amizade com o Líder (1000001)
            if GUILDA_ID in db["guilds"]:
                db["guilds"][GUILDA_ID]["name"] = NOME_GUILDA
                if player_id not in db["guilds"][GUILDA_ID]["members"]:
                    db["guilds"][GUILDA_ID]["members"].append(player_id)
                    print(f"+++ [GUILDA] {db['players'][player_id]['nickname']} ({player_id}) adicionado à {NOME_GUILDA}! +++")
            ...
            response = {
                ...
                "session_key": "guelff_v14dv13_render_" + hashlib.md5(player_id.encode()).hexdigest()[:8]
            }
    ...
    def do_GET(self):
        db = self.get_db()
        if "ver.php" in self.path or "version" in self.path or "216.24.57.1" in self.path:
...
if __name__ == "__main__":
    print(f"====================================================")
    print(f"   {SERVER_NAME} - RENDER 24H")
    print(f"   GUILDA: {NOME_GUILDA}")
    print(f"   PORTA: {PORT} | ACESSO: LIBERADO (SEM TRAVA IP)")
    print(f"====================================================")
